...
[[ $EUID -ne 0 ]] && echo -e "${red}Error:${plain} This script must be run as root!${red}错误:${plain} 这个脚本必须以root权限运行!" && exit 1

[[ -d "/proc/vz" ]] && echo -e "${red}Error:${plain} Your VPS is based on OpenVZ, not be supported.${red}错误:${plain} 你的系统基于openvz架构, 不被支持." && exit 1
...
echo
read -p "Info: The system needs to be restart. Do you want to reboot? 系统需要重启后bbr生效，是否重启？[y/n]" is_reboot
